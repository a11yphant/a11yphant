...
@Injectable()
export class ElementNotExists implements Check {
  ...
  public async run(submission: Submission, rule: Rule): Promise<RuleCheckResult> {
    ...
    try {
      ...
      this.logger.error(`Executing check ${rule.key} on submission ${submission.id} failed: ${error.message}`, error.stack, ElementNotExists.name);
