...
const CollapsibleSection: React.FunctionComponent<CollapsibleSectionProps> = ({ classes }) => {
  ...
  return (
    <div className={`${classes} flex flex-col items-center w-full box-border h-fitContent max-w-7xl m-auto`}>
      <div className="flex flex-row items-center w-full box-border py-6">
        <div className="flex flex-col w-full">
          <h3>
            <Button
              className="flex flex-row-reverse m-0 p-0 group text-white font-bold transition duration-300 hover:text-primaryDark focus:text-primaryDark"
              overrideClassname
              aria-expanded="false"
              icon={<Chevron className="text-white mr-8 group-hover:text-primaryDark group-focus:text-primaryDark" />}
            >
              1.0 Global Requirements
            </Button>
          </h3>
          <div>
